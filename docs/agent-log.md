...
## 2026-07-20 — Claude Code: 方針決定(ドラクエ的な見た目への寄せ方)

**背景**
- ユーザーから「今の構成(UI・ゲームエンジン・デザイン)は最適か?」という相談があり、Claude Codeから2点を課題として提示した:
  1. バトルUI(`battle_controller.gd`)がPanel/Labelの座標をコードで直書きして構築されており(`.tscn`シーンやアンカー/コンテナ未使用)、見た目の微調整がエディタ上でできずコード→デプロイ→ブラウザ確認のサイクルが必要になっている。
  2. 前回ログ済みの「2.5D風背景 vs 2D矩形の当たり判定」の不一致(下記の前回エントリ参照)。
- ユーザーの回答: 「ドラクエ的な感じにできればいいや」。方向性としては採用、ただし**今回は実装せず方針をログに残すだけ**という指示。

**方針(次のセッション以降で検討・着手する内容)**
- [ ] **バトルUIのドラクエ風化**: 画像生成不要、コードのみで着手可能。青系グラデーション+白ピクセル枠の伝統的なコマンドウィンドウ、縦一列コマンドリスト、独立したメッセージウィンドウ(下段に固定)といった構成に寄せる。`_style()`/`_panel()`のStyleBoxFlatパラメータとレイアウト定数を調整する形になりそう。
- [ ] **マップ美術・当たり判定のドラクエ風(フラット2D)化**: ドラクエの見下ろし型マップは基本的にフラットな2Dタイルなので、これは前回ログの当たり判定不一致問題とも方向性が一致する。ただし`office_front`などの既存背景画像を描き直す作業は画像生成が必要で、Claude Code環境には画像生成ツールがない → Codexに依頼する想定。Claude Code側では「新規に追加するマップ/アセットは最初からフラット2D構図で作る」というルールをどこかに明文化しておくと、今後同種の齟齬を防げる。
- [ ] 上記2点のどちらを先にやるかはまだユーザーと合意していない(前回提案では画像生成不要なUI側を優先候補として提示したが、確定はしていない)。次のセッション開始時にユーザーへ確認すること。
