...
## 2026-07-19 — Claude Code: 未着手の問題を記録(2.5D背景と当たり判定の不一致)

**問題**
- ユーザーが実プレイで発見: 背景美術が2.5D風(奥行きのある構図)なのに、当たり判定(壁)は単純な軸並行矩形の2Dのままで、見た目と当たり判定がずれている。
- 調査の結果、`godot/data/maps.json` の `walls` 配列の数を全マップで比較すると偏りがあった:
  - `office_front`(社屋前、最初に表示される画面): 壁4枚 = マップ外周のみ。背景画像(`office-front-v2.png`)に描かれた建物本体・木・自販機・駐輪場に対応する当たり判定が一切ない。
  - `tanuki`/`matsu`/`snack`: 同じく壁4枚(ただしこの3店は`background`未設定で、まだ旧来のフラットな図形描画のため2.5D画像自体の問題ではない)。
  - `office_inside`(9)/`station`(8)/`home`(9)/`yokocho`(5)/`santa`(7)/`coffee`(10): 建物内部の間仕切りなどに対応した壁が既にある程度用意されている。
  - → `office_front` が「詳細な2.5D背景があるのに壁データが追いついていない」の最も明確な該当箇所。プレイヤーが最初に触る画面でもあるため優先度が高い。

**次にやること**
- [ ] `office_front` の背景画像(建物ファサード、左右の街路樹、赤い自動販売機、駐輪場)に対応する壁(StaticBody2D)を追加する。既存の出入口ポータル(`office_front_to_inside`, 位置`[480,325]`)への接近経路は塞がないよう隙間を残すこと。
- [ ] 他のマップ(特に`station`/`home`/`yokocho`/`coffee`/`santa`)も、背景画像と壁データの整合性を一通り目視確認する(壁の数はあっても位置がずれている可能性は未検証)。
- [ ] 修正後は `validate_data.gd` に加え、実際にブラウザで壁際を歩いて視覚的な建物・木・自販機を通り抜けられないか確認すること(このログの前のエントリで学んだ通り、レスポンスが返る・データが通るだけでは不十分)。
